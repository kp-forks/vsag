...
main(int argc, char** argv) {
    ...
    if (not index_clone_result.has_value()) {
        std::cerr << "Index Clone failed: " << index_clone_result.error().message << std::endl;
    }
    auto index_clone = index_clone_result.value();

    /******************* Compare KnnSearch Result *****************/
    auto result_clone = index_clone->KnnSearch(query, topk, hgraph_search_parameters);
    if (not result_clone.has_value()) {
        std::cerr << "Search Error: " << result_clone.error().message << std::endl;
    }
    ...
    std::cout << "clone index results: " << std::endl;
    for (int64_t i = 0; i < result_clone.value()->GetDim(); ++i) {
        std::cout << result_clone.value()->GetIds()[i] << ": "
                  << result_clone.value()->GetDistances()[i] << std::endl;
    }
